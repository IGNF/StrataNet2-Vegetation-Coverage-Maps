...
class KdeMixture:
    ...
    def plot_kde_mixture(self, x_lim):
        ...
        # ax.set_title(f"Mixture of KDE for Vb, Vi, Vh")
        ax.plot(X, y1, label="Elévation basse", color="green")
        ax.plot(X, y2, label="Elévation intermédiaire", color="blue")
        ax.plot(X, y3, label="Elévation haute", color="black")
        ax.set_xlim([0, x_lim])
        ax.set_ylim([0, 1.2])
        ax.legend()
        plt.tight_layout()
