...
parser.add_argument('--corrected_gt_file_path', default=os.path.join(data_path, "placettes_dataset_correction/placettes_metadata_correction_210712_formatted.csv"), type=str, help="Path to ground truth file. Put in dataset folder.")
...
parser.add_argument('--n_epoch_test', default=50 if not mode=="DEV" else 1, type=int, help="We evaluate every -th epoch, and every epoch after epoch_to_start_early_stop")
parser.add_argument('--epoch_to_start_early_stop', default=280 if not mode=="DEV" else 1, type=int, help="Epoch from which to start early stopping process, after ups and down of training.")
